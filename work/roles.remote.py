...
class NurserySSO(http.Controller):
    ...
    def _month_totals(self, month):
        ...
        if excel_active:
            ...
            books_paid_total = sum(float(s.books_paid or 0.0) for s in students)
            books_remaining_total = max(0.0, books_due_total - books_paid_total)
            books_unpaid_count = len(students.filtered(
                lambda s: (s.books_paid or 0.0) < (s.books_fees or BOOKS_FEE_DEFAULT) - 0.01))
    ...
    def api_manager_month_get(self, mt=None, ym=None, **kw):
        ...
        return {
            ...
            'fees': [{
                ...
                'remaining': round(max(0.0, (f.fees or 0.0) +
